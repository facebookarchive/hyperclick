...
export default class Hyperclick {
  ...
  constructor() {
    ...
    this._textEditorSubscription = atom.workspace.observeTextEditors(item => {
      ...
      const uri = item.getPath();
      const isBrokenDeserializedUri =
        uri != null && uri.match(/nuclide:[\\/][^/]/) != null;
      if (isBrokenDeserializedUri) {
        return;
      }
      return this.observeTextEditor(item);
    });
  }
